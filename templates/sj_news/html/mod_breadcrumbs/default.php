...
<nav class="mod-breadcrumbs__wrapper" aria-label="<?php echo htmlspecialchars($module->title, ENT_QUOTES, 'UTF-8'); ?>">
	<ul itemscope itemtype="https://schema.org/BreadcrumbList" class="mod-breadcrumbs breadcrumb px-3 py-2">
		...
		<?php else : ?>
			<li class="mod-breadcrumbs__divider float-start active"><!--i class="icon-home fa fa-home"></i--></li>
		<?php endif; ?>
		...
		foreach ($list as $key => $item) :
			if ($key !== $last_item_key) :
				if (!empty($item->link)) :
					$breadcrumbItem = '<a itemprop="item" href="' . Route::_($item->link) . '" class="pathway">' . html_entity_decode($item->name, ENT_QUOTES, 'UTF-8') . '</a>';
				else :
