...
if(!empty($list)){
	...
	SjBasicNewsBaseImageHelper::setDefault($params);
?>
	...
	<div id="<?php echo $uniquied?>" class="sj-basic-news">
		<div class="bs-items bsn-wrap theme2col">
			<?php  $i = 0;  foreach($list as $item){ $i++;
				$show_line = ($params->get('showline') == 1)?' bs-show-line':'';
				$last_class = ($i == count($list))?' last last-item':'';
				$first_class = ($i == 1)?' first-item':'';
				$img = SjBasicNewsHelper::getAImage($item, $params);
			?>
			<div class="bs-item item post cf <?php echo $show_line.' '.$last_class.$first_class; ?>">
				<?php if($img){?>
				<div class="bs-image bsn-image shadow">
				...
				<div class="bs-content">
					...
					<?php if($params->get('item_date_display') == 1 ){?>
						<span class="bs-date basic-date"><?php echo  JText::sprintf( JHTML::_('date',$item->created, 'M d Y')); ?></span>
					<?php }?>
						
					<?php if($params->get('item_desc_display') == 1) {?>
					<p class="bs-description basicnews-desc">
						<?php echo SjBasicNewsHelper::truncate(strip_tags($item->introtext), $params->get('item_desc_max_characs',200)); ?>
					</p>
					<?php } ?>
					...
					<?php if($params->get('item_readmore_display') == 1){?>
						<a class="more-view" href="<?php echo $item->link ?>" title="<?php echo $item->title; ?>" <?php echo SjBasicNewsHelper::parseTarget($params->get('link_target')); ?>>
							<span><?php echo $params->get('item_readmore_text'); ?></span>
						</a>
					<?php }?>
				</div>
			</div>
