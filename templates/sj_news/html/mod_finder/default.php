...
$input = '<input type="text" name="q" id="mod-finder-searchword' . $module->id . '" class="inputbox search-query js-finder-search-query form-control" value="' . htmlspecialchars($app->input->get('q', '', 'string'), ENT_COMPAT, 'UTF-8') . '"'
...
{
	$output .= $label;
	$output .= '<div class="finder clearfix">';
	$output .= $input;
	$output .= '<button class="button btn btn-primary" type="submit">'. JText::_('JSEARCH_FILTER_SUBMIT') . '</button>';
	$output .= '</div>';
}
else
{
...
<form id="mod-finder-searchform" class="mod-finder js-finder-searchform form-search" action="<?php echo Route::_($route); ?>" method="get" role="search">
	<?php echo $output; ?>
