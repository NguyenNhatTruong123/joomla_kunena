...
define ('YT_PLUGIN_SHOPRT', 'ytshortcodes');
define ('YT_PLUGIN_SHOPRT_REL_URL', 'plugins/system/'.YT_PLUGIN_SHOPRT);
...
class plgSystemYtshortcodes extends JPlugin{
	...
	public function onBeforeRender()
	{
		...
		$f_filePath = JPath::clean(JPATH_ROOT.'/'.YT_PLUGIN_SHOPRT_REL_URL.'/'.YT_PLUGIN_SHOPRT.'.xml');
		...
		if($this->params->get('show_sjbootstrap', 0)==1 && $app->isClient('site')){
	...
	public function ytStyleSheet($url){
		...
		if(($app->getTemplate(true)->params->get('developing', 0)==1 || $app->input->getVar('less2css')=='all') && file_exists($lessurl)){
	...
	public function addBtnShortCodes()
	{
		...
		}else
		{
			...
			if($editor == "tinymce")
			{
				$page = str_replace('<div class="toggle-editor btn-toolbar float-end clearfix mt-3">','<div id="editor-xtd-buttons" class="toggle-editor btn-toolbar float-left clearfix mt-3">'. $button.'</div><div class="toggle-editor btn-toolbar float-end clearfix mt-3">', $page);
