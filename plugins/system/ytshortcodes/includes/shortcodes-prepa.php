...
function accItemShortcode($atts, $content = null){
	...
	$acc_item = $acc_item . "<div class='accordion-inner'>" . parse_shortcode(str_replace(array("<p></p>"), " ", $content)) . "</div>";
...
function galleryShortcode($atts, $content = null){
	...
	JHtml::stylesheet(JUri::base()."plugins/system/ytshortcodes/assets/css/jquery.fancybox.css");
	$script_prettyphoto  = JHtml::script("plugins/system/ytshortcodes/assets/js/jquery.fancybox.js");
	extract(ytshortcode_atts(array(
		"title" 	=> '',
		"width"		=> 100,
		"height"	=> 100,
...
function galleryItemShortcode($atts, $content = null){
	...
	$gallery_item .= "<a title='" . $title . "' href='" . $src_pop . "' data-fancybox>";
...
function lightboxShortcode($atts){
	...
	if($lightbox == 'On' || $lightbox == 'on') {
		$frame = "<a href='". $src . "' data-fancybox title='" . $title . "' >" . $frame . $titles. $borderinner. "</a>";
	}
...
function vimeoShortcode($atts, $content = null){
	...
	$vimeo = $vimeo . '<iframe src="//player.vimeo.com/video/' . $id[1] . '?title=0&amp;byline=0&amp;portrait=0" frameborder="0" width="' . $width . '" height="' . $height . '" ></iframe>';
	$vimeo = $vimeo . '</div>';
...
function youtubeShortcode($atts, $content = null){
	...
	$youtube = $youtube . '<iframe src="//www.youtube.com/embed/' . $id[1] . '?rel=0&autohide=1&showinfo=0" width="' . $width . '" height="' . $height . '" frameborder="0" allowfullscreen></iframe>';
	$youtube = $youtube . '</div>';
