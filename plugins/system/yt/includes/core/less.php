...
class YTLess extends lessc{
	...
	public static function addStylesheet($pfile){
		...
		if(strtolower($compileLess) == 'client' && $app->input->getVar('compile')!='server'){
			// add rel = "stylesheet/less"
			$doc->addHeadLink($pfile, 'stylesheet/less');
		}else{
			if( $app->input->getVar('less2css')=='all' ){			
				...
				if(basename($pfile)=='template.less'){
					...
					for($i=0; $i<count($dircolor); $i++){
						...
						if(is_file(dirname($pfile).'/color/'.$dircolor[$i])){
							$tcontent = file_get_contents(JPATH_ROOT.'/'.dirname($pfile).'/color/'.$dircolor[$i]);
	...
	public static function buidStyleSheet($pfile){
		...
		$cssfile = $app->getTemplate(true)->params->get('optimizeFolder', 'yt-assets').'/developing/'.$pfile;
	...
	public static function hasNew($pfile, $cssurl, $pthfile){
		$content = file_get_contents($pthfile);
	...
	public static function complieToCss($path, $topath){
		...
		foreach ($arrImport as $s) {
			if ($import) {
				...
				$importcontent = file_get_contents(JPATH_ROOT.'/'.$url);
