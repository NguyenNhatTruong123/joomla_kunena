...
abstract class YtFrameworkTemplate {
	...
	function __construct($template =null, $cookie){
		$this->_tpl = $template;
		...
		if(is_file(JPATH_ROOT.'/templates/'.$this->template.'/less/color/'. $this->getParam('templateColor').'.less')){
			$colorcontent1 = file_get_contents('templates/'.$this->template.'/less/color/variables_color.less');
			$colorcontent2 = file_get_contents('templates/'.$this->template.'/less/color/'. $this->getParam('templateColor').'.less');
	...
	function getUserSetting(){
		...
		if (isset($_COOKIE[$this->template.'_tpl']) && $_COOKIE[$this->template.'_tpl'] == $this->template){
			foreach($this->_params_cookie as $k=>$v) {
				...
				if ($app->input->getVar($k)!= null){
					$v = $app->input->getVar($k);
					@setcookie ($kc, $v, $exp, '/');
				}else{
					if (isset($_COOKIE[$kc])){
						$v = $_COOKIE[$kc];
	...
	function isHomePage(){
		...
		$query = "SELECT home FROM #__menu WHERE id=" . intval($input->getCmd( 'Itemid' ))."";
	...
	function isOverrideTemplate(){
		$app = JFactory::getApplication();
		if($app->input->getVar('tmpl')!='component'){ //var_dump(JSite::getMenu()->getActive()); die();
			$menuid = '';
	...
	function ytStyleSheet($url){
		...
		if(($this->getParam('developing', 0)==1 || $input->getVar('less2css')=='all') && file_exists($lessurl)){
			YTLess::addStyleSheet($lessurl);		
		}elseif(file_exists($url)){
			$doc->addStyleSheet($url);
		}elseif(basename($url)=='template.css'){
			if(isset($_COOKIE[$this->template.'_themecolor'])){
				$doc->addStyleSheet(str_replace('template.css', 'template-'.$_COOKIE[$this->template.'_themecolor'].'.css', $url));
			}else{
	...
	function getLogo(){
		...
		if ($this->getParam('logoType')=='image'):  
			if($this->getParam('overrideLogoImage')!=''):
				...
				if(is_file('templates/'.$this->template.'/images/'.$this->getParam('themecolor').'/logo.png')){
					$url = $this->templateurl().'images/'.$this->getParam('themecolor').'/logo.png';
				}else{
	...
	function renPositionsGroup($position, $type='', $special = null){
		...
		if($position['group']=='main'){
			...
			$prefx = isset($special['mainprefix']) ? $special['mainprefix'] : '';
		}else{
			$prefx = '';
		}
		// Element class
